...
def plot_dataset(
    data, file_format="pdf", size=5, cmap=None, desc=None, shape=None, sort=True, save=True, name=None, titles=False
):
    if sort:
    ...
    filename = f"dataset_{datetime.now().strftime('%Y%m%d_%H%M%S')}{'_' + desc if desc is not None else ''}" if name is None else name
    #plt.tight_layout()
